...
class BimAktuel(Aktuel):
    ...
    def getFutureAktuels(self, page_content):
        aktuels = []
        try:
            futures = page_content.find("div", { "class" : "subButtonArea-5" }) #https://stackoverflow.com/a/40305745
            for brosur in futures.find_all('a','subButton'): 
                aktuel = {'magaza':'BİM','aktuel':"",'durum':'','tarih':''}
    ...
    def getCurrentAktuels(self, page_content):
        aktuels = []
        try:
            currents = BeautifulSoup(str(page_content.select('div.subButtonArea.active')),"lxml") #https://stackoverflow.com/a/40305745
...
class A101Aktuel(Aktuel):
    ...
    def getCurrentAktuels(self, page_content):
        aktuels = []
        try:
            currents = page_content.find("div","brochures-list").find("ul", recursive=False)
            for brosur in currents.find_all("li", recursive=False):
                aktuel = {'magaza':'A101','aktuel':"",'durum':'','tarih':''}
                aktuel['aktuel'] = brosur.find("span").text #+ ' ' + brosur.a['href']
                aktuel['durum'] = 'new'
                aktuel['tarih'] = str(datetime.datetime.now())
                aktuels.append(aktuel)
        except Exception as error:
        ...
        try:
            ...
            for brosur in currents:
                ...
                if filename is not None:
                    aktuel = {'magaza':'ŞOK','aktuel':"",'durum':'','tarih':''}
